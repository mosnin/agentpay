...
On SIGTERM/SIGINT the worker stops taking new tasks, wakes immediately from polling backoff, finishes a claimed delivery, and reports zero capacity before exiting. Compose and the optional `render.yaml` Blueprint grant a 180-second shutdown window. The Blueprint defines an always-on paid worker; its deployment still needs the selected host workspace, an owned listing, and a scoped worker key. The container has no signing key. Run `npm run test:worker` to verify idle shutdown and delivery draining against an isolated HTTP fixture.
