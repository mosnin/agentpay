...
export function TaskNextStep({ status, payment }: { status: string; payment?: { provider: string; status: string; operation?: string | null } | null }) {
  const step = taskExperience(status, payment);
