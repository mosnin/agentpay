...
## Engineering continuation — operational delivery

Implemented after 2972ce6:

- Scheduler endpoints persist leases, fenced run history and interruption recovery. The operations screen flags missing/failed runs, exhausted notification attempts and halted chains. Eight-attempt notification dead letters can be retried by an administrator with an audit record; retry never sends another payment. A five-minute GitHub workflow and repository credentials target the isolated release preview. GitHub scheduled execution starts only when the workflow reaches the default branch; manual endpoint verification is recorded separately.
- Newly issued API keys have explicit worker/read-only/full-account profiles and 7/30/90-day expiry. Worker keys cannot purchase, approve buyer payment, manage account settings or inherit admin privileges. Both route gates and server actions enforce scope. Seller settlement access permits artifact submission and receipt reconciliation. Existing full-access keys remain identifiable as legacy credentials. Active keys remain reachable even after extensive expired/revoked history.
- Authenticated per-agent heartbeat/capacity and an owned-listing readiness endpoint distinguish listings from connected workers. The reference service reports capacity, backs off after transport failures and has a non-root container/supervisor definition. Heartbeats describe reports, not verified successful execution.
- Account-persisted briefs recover after refresh; revision checks prevent stale tabs overwriting new work. Repeat tasks copy editable terms, reset the deadline and use a new creation key. Concurrent identical creation requests recover the same agreement instead of duplicating work. Saved agents are account-scoped and paginated.
- Owned-agent management and administrative agent/dispute search are paginated with full matching counts. Public reviews, personal trust findings and moderation appeals have complete paginated history. Search infrastructure now deploys through migrations; handlers no longer issue DDL. The full-text expression matches its index, with capability matching separate.
- Support combines private service reports, task disputes and trust appeals. Users can file reports and track responses; administrators record review/resolution with audit entries. Existing independent-reviewer rules remain in force for trust appeals. Form failures preserve the user's text.
- First live delivery conversion shows successful buying accounts over all requesting accounts. Production rate limits use atomic PostgreSQL buckets when Redis is absent; concurrent sensitive requests share one limit and fail closed on store failure. Local/demo tests retain their bounded in-memory limiter.
- A bounded hosted HTTP probe measures reads and concurrent idempotent writes, rejects invalid credentials and checks cross-account denial. An isolated preview backup was restored locally and six core table counts matched. The backup was of a nearly empty preview (one user), so this is restore-mechanism evidence, not a production-volume recovery-time guarantee.

Release constraints remain explicit: the public Base Sepolia runner still finds zero native gas in the temporary actors and 20 test USDC in the buyer. No public-chain contract or mainnet fee wallet has been represented as deployed. Hosted payment-provider acceptance, a persistent deployed seller with actual funded delivery, sustained traffic/DB-pool saturation and production rollback acceptance remain distinct from these engineering and preview checks.

Verification at e897311: 307 unit tests, 6 database recovery/concurrency tests, 8 scale tests and 30 browser tests passed. Hosted CI run 34167447852 passed all four jobs, including real local-chain settlement recovery and the funded browser journey. The protected Vercel preview passed a 24-request probe at concurrency 1/5/10 (p95 1,534 ms): five competing creation requests produced one pending agreement, and a separate account was denied access. Both authenticated scheduler endpoints returned 200 and persisted successful run records; three PostgreSQL rate-limit buckets were observed. No network was enabled during that scheduler test. Unpaid acceptance fixtures and short-lived API keys were removed afterward. Preview alias rollback to 2972ce6 and forward restore both returned healthy database responses. Production and public-chain acceptance are not implied.

Visual repair from review: shorten the API-key permission labels and explain the selected authority beneath the control; preserve the selected moderation tab when paging. The first brief/dialog captures caught loading/entrance states and are replaced by captures that wait for the input and settled dialog.
