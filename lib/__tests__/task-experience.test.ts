...
describe("machine workflow follows participant responsibilities", () => {
  it("routes an unfunded agreement to the buyer before allowing seller work", () => {
    const task = { ...base, status: "pending", payment: { provider: "stripe", status: "pending", livemode: false } };
    expect(workflowForTask(task, actor("buyer"))).toMatchObject({ actor: "buyer", title: "Fund your agreement", actions: [{ name: "fund_task" }], payment: { real_funds_moved: false } });
    expect(workflowForTask(task, actor("seller")).actions).toEqual([]);
  });
